...
My results show that under the aggressive reward style ($n=5$ seeds), PPO with
...
deterministic policies and my reward styles differ only in reward coefficients,
...
on this benchmark are folklore; I measure them under identical conditions.
\end{enumerate}
...
not my claim to novelty. My contribution is an \textbf{empirical finding with
methodological teeth}: under a controlled, parameter-fair comparison I show that
\emph{episodic return---the metric almost universally reported on this
benchmark---can be actively misleading for safety-critical control}, and I
...
PPO). I replicate this on a \emph{second} environment (\code{merge-v0}), where
returns tie within $1.4\%$ but PPO crashes in $100\%$ of episodes against DQN's
$0\%$. I argue this makes \textbf{behaviour-level evaluation a necessary
...
\textbf{How this study came about.} I did not set out to study evaluation
metrics. I started by reproducing the standard attention baseline as a warm-up,
expecting the architecture comparison to be the story. What redirected the
project was an accident of observation: the three configurations reached almost
the same episodic return, but when I actually watched them drive they behaved
nothing alike---one cruised, another drove straight into traffic. That mismatch
between ``the number'' and ``the behaviour'' is what this paper ended up being
about.
...
My use of it is a \emph{reproduction}, used here as one arm of a controlled
ablation.
...
dynamics, and reward. I use the \code{Kinematics} observation and the
...
\textbf{Episode horizon (a subtle point I learned the hard way).} I initially
assumed \code{duration}~$=80$ meant 80 decision steps, and my early reward
numbers refused to add up---episodes seemed to end ``too late'' for the per-step
rewards I was logging. Tracing \code{self.time} in the source resolved it:
...
collisions and lane-change penalties. I expose three \emph{driving styles} that
change the reward coefficients (Table~\ref{tab:reward}). These coefficients were
not chosen analytically but tuned by hand until the behaviour matched the label:
for the aggressive style I first kept the default collision penalty of $-1.0$,
but with \code{high\_speed\_reward}$=2.0$ the agent simply ignored collisions and
floored it; only after raising the penalty to $-4.0$ did crashing start to
register in its decisions. Crucially, the \textbf{absolute return scale differs
...
learning rate \textbf{linearly decayed} to zero. The detail that cost me the
most debugging time was the GAE termination handling: my first version
normalised advantages over the whole rollout and dropped the per-step
\code{nextnonterminal} mask, so advantage estimates leaked across episode
boundaries and the learning curve was erratic. Restoring the per-step done mask
and moving advantage normalisation \emph{inside} each minibatch (as in the final
code) is what made training stable.
...
ablation is parameter-fair. I stress the scope of this fairness: it controls
...
controlled-capacity one. I keep DQN small deliberately, as a lightweight
...
\textbf{Reproducibility cross-check.} I additionally train \textbf{SB3
PPO}~\cite{sb3} (\code{MlpPolicy}) with hyperparameters matched to my
...
methods, I compare \emph{behaviour}. For each model I run \textbf{30
...
dynamics. I verified in the \code{highway-env} source that my driving
...
under the base or aggressive coefficients. I report them under the base
...
collapse together: under the aggressive style the high-speed term pays $+2.0$
\emph{every step} the agent is fast, while a collision is penalised only $-4.0$
\emph{once}. Over a 400-step horizon the return is therefore dominated by a
speed integral, and a late crash barely dents it---so episodic return is almost
blind to exactly the failure mode we care about. This is the first hint of why
return and behaviour decouple so sharply in Section~\ref{sec:behav}.
...
$368.5$ average survival steps. I caution against an uncritical reading of this,
and the caution comes from actually watching the policy: when I rendered the DQN
agent to video, it was not smoothly steering around obstacles but jittering
left--right almost every step. DQN indeed changes lanes \emph{far} more than any
other model ($155$ per episode on average, with two seeds exceeding
$260$---near-continuous lane switching in a $400$-step episode). Because the
aggressive \emph{training} reward \emph{positively} rewards lane changes ($+0.2$;
Table~\ref{tab:reward}), the policy is actively incentivised to switch lanes, so
the low crash rate may be partly an artifact of a degenerate
``swerve-to-survive'' strategy rather than a genuinely intelligent avoidance
policy. I therefore release the rendered video so the reader can judge this
directly rather than take the crash number at face value. In contrast, \textbf{PPO+Attention and PPO+MLP exhibit
...
I added this cross-check for a concrete reason: my PPO+Attention seed 2 came in
at $207.0$, well below the other seeds, and I could not immediately tell whether
that was ordinary seed variance or a bug in my hand-written PPO. Running a
trusted reference implementation under matched hyperparameters was the cheapest
way to find out. Because SB3 uses an \code{MlpPolicy} $[64,64]$ trunk, its
closest architectural counterpart is my custom \textbf{PPO+MLP}
($223.0 \pm 19.6$), not the attention model. The SB3 run (seed 0, hyperparameters matched to my
implementation) reached a final rolling-average reward of \textbf{260.5} and a
peak of \textbf{266.7}. This single-seed value exceeds \emph{every} one of my
custom PPO seeds (best individual: PPO+Attention seed 1, $255.3$; best PPO+MLP
seed, $248.4$) and sits ${\approx}17\%$ above my PPO+MLP 5-seed mean. I read
this gap not as evidence that my PPO is broken---my implementation reaches the
...
normalisation, tuned optimiser $\epsilon$) that I did not all replicate.
Because SB3 is a single seed against my five, this is a directional sanity
...
\label{sec:styles}
Beyond the core ablation I trained PPO+Attention under the \emph{conservative}
...
\label{sec:merge}
The objection I most expected from a reader is that the headline finding---return
parity hiding a behavioural gap---might be an artifact of \code{highway-v0}
alone. I chose \code{merge-v0} to pre-empt it precisely because its dynamics are
the \emph{least} like highway: instead of free-flowing multi-lane cruising, the
ego must time a merge from a short on-ramp onto a highway with an obstacle at the
ramp end---a different traffic topology and failure mode. If the finding survived
there, ``it only holds on highway'' would be hard to maintain. I repeated the
core experiment with the identical training pipeline and aggressive reward style
...
than principled avoidance---my supplementary video lets the reader judge. With
...
caveat of Sec.~\ref{sec:behav}). I attribute the \emph{training stability} to the
...
are evaluated deterministically and my reward styles differ only in reward
...
slower policy that does not. I attribute DQN's \emph{training} stability to
experience replay (above); I are careful, however, not to over-claim DQN as
...
\textbf{Implementation correctness.} My custom PPO reaches the same reward
ceiling ($266.7$ peak) and the same qualitative learning dynamics as the SB3
reference, supporting the correctness of my GAE, clipped surrogate, advantage
normalisation, and linear LR decay. I are deliberately cautious here: SB3's
single-seed $260.5$ exceeds all of my PPO seeds, so I claim only that my
...
claims; a formal test (e.g.\ Mann-Whitney $U$) requires more runs. I therefore
...
on \code{highway-v0} ($n=5$); I corroborate the central finding on
\code{merge-v0} ($n=3$, Sec.~\ref{sec:merge}) but do not extend to
...
minutes wall-clock (the spread depends on how many runs I had going at once
...
in the core comparison were trained under the aggressive style; I do not test
...
very frequent lane changes; I cannot fully separate intelligent avoidance from
a degenerate swerving policy without further trajectory analysis (Sec.~\ref{sec:behav}).
\end{itemize}

\section{Conclusion}
I presented a from-scratch reproduction and parameter-matched architectural
...
steps), which I attribute to experience replay smoothing gradient variance. I
...
\emph{GitHub repository}, 2018. https://github.com/czh513/Auto-driving-RL-decision-making
